...
describe("EMPTY_STATE_COPY", () => {
  ...
  it("matches documented pattern (title, description, action)", () => {
    ...
    expect(EMPTY_STATE_COPY.sketch.description).toBe(
      "Describe your app idea and Open Sprint will generate a PRD."
    );
    expect(EMPTY_STATE_COPY.plan.title).toBe("No plans yet");
    expect(EMPTY_STATE_COPY.plan.description).toBe(
      "Create a plan to break down your spec into tasks."
    );
    expect(EMPTY_STATE_COPY.execute.title).toBe("No tasks yet");
    expect(EMPTY_STATE_COPY.execute.description).toBe(
      "Ship a plan from the Plan phase to start generating tasks."
    );
    expect(EMPTY_STATE_COPY.eval.title).toBe("No feedback yet");
    expect(EMPTY_STATE_COPY.eval.description).toBe(
      "Test your app and report findings using the form above."
    );
    expect(EMPTY_STATE_COPY.deliver.title).toBe("No deliveries yet");
    expect(EMPTY_STATE_COPY.deliver.description).toBe(
      "Configure targets in settings, then deploy from the toolbar."
    );
  });
});
