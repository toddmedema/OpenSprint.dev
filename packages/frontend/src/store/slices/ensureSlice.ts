...
export const fetchFeedback = createAsyncThunk("ensure/fetchFeedback", async (projectId: string) => {
  return api.feedback.list(projectId);
});

export const submitFeedback = createAsyncThunk(
  "ensure/submitFeedback",
  async ({
    projectId,
    text,
    images,
...
const ensureSlice = createSlice({
  ...
  reducers: {
    ...
    setEnsureError(state, action: PayloadAction<string | null>) {
...
export const { setFeedback, setEnsureError, resetEnsure } = ensureSlice.actions;
export default ensureSlice.reducer;
