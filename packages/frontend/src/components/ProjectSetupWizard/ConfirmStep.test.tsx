...
function renderConfirmStep(overrides: Partial<Parameters<typeof ConfirmStep>[0]> = {}) {
  return render(
    <ConfirmStep
      ...
      planningAgent={{ type: "cursor", model: "gpt-4", cliCommand: null }}
...
describe("ConfirmStep", () => {
  ...
  it("shows planning and coding agent labels", () => {
    renderConfirmStep({
      planningAgent: { type: "cursor", model: "gpt-4", cliCommand: null },
      codingAgent: { type: "claude-cli", model: "claude-3-5-sonnet", cliCommand: null },
    });
  ...
  it("shows custom agent label when cliCommand is set", () => {
    renderConfirmStep({
      planningAgent: { type: "custom", model: null, cliCommand: "my-agent --foo" },
    });
