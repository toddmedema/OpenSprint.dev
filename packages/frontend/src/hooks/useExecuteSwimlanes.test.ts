...
describe("useExecuteSwimlanes", () => {
  it("filters out epics and gating tasks from implTasks", () => {
    ...
    const { result } = renderHook(() =>
      useExecuteSwimlanes(tasks, plans, "all", "")
    );
  ...
  it("groups tasks by epic into swimlanes", () => {
    ...
    const plans: Plan[] = [
      plan({
        content: "# Epic A\n",
        metadata: { planId: "p1", beadEpicId: "epic-a", gateTaskId: "epic-a.0", shippedAt: null, complexity: "medium" },
      }),
      plan({
        content: "# Epic B\n",
        metadata: { planId: "p2", beadEpicId: "epic-b", gateTaskId: "epic-b.0", shippedAt: null, complexity: "medium" },
      }),
    ];
    const { result } = renderHook(() =>
      useExecuteSwimlanes(tasks, plans, "all", "")
  ...
  it("filters by search query", () => {
    ...
    const plans: Plan[] = [plan()];
    const { result } = renderHook(() =>
      useExecuteSwimlanes(tasks, plans, "all", "Login")
    );
