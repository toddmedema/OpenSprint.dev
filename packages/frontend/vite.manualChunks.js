...
export function getManualChunkForModuleId(id) {
    const normalizedId = id.replaceAll("\\", "/");
    if (!normalizedId.includes("/node_modules/")) {
        return;
    }
    if (isNodeModule(normalizedId, "react") ||
        isNodeModule(normalizedId, "react-dom") ||
        isNodeModule(normalizedId, "scheduler")) {
        return "vendor-react";
    }
    if (isNodeModule(normalizedId, "react-router") ||
        isNodeModule(normalizedId, "react-router-dom")) {
        return "vendor-router";
    }
    if (isNodeModule(normalizedId, "@reduxjs/toolkit") ||
    ...
    if (isNodeModule(normalizedId, "@tanstack/react-query") ||
    ...
    if (isNodeModule(normalizedId, "react-markdown") ||
    ...
    if (isNodeModule(normalizedId, "mermaid") ||
