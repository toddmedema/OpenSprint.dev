...
export type FailureType = "test_failure" | "review_rejection" | "agent_crash" | "repo_preflight" | "environment_setup" | "timeout" | "no_result" | "merge_conflict" | "merge_quality_gate" | "coding_failure";
 ...
 */
export declare function getQualityGateFailureLabel(failureType: "environment_setup" | "merge_quality_gate"): string;
...
export declare const REPO_PREFLIGHT_REMEDIATION_GIT = "Fix repository git setup (base branch and git identity), then retry.";
/** Remediation for repo preflight when the failure is dependency-related. */
export declare const REPO_PREFLIGHT_REMEDIATION_DEPENDENCY = "Run npm ci in the repository root, then fix invalid dependencies before retrying.";
/** Remediation for environment_setup failure. */
export declare const ENVIRONMENT_SETUP_REMEDIATION = "Run npm ci in the repository root, re-link worktree node_modules, then retry.";
/** Remediation constants (aliases for compatibility). */
export declare const REMEDIATION_ENVIRONMENT_SETUP = "Run npm ci in the repository root, re-link worktree node_modules, then retry.";
export declare const REMEDIATION_PREFLIGHT_DEPENDENCY = "Run npm ci in the repository root, then fix invalid dependencies before retrying.";
export declare const REMEDIATION_PREFLIGHT_GIT = "Fix repository git setup (base branch and git identity), then retry.";
/**
 * Returns the remediation message for repo_preflight or environment_setup.
 */
export declare function getRemediationForFailureType(failureType: "repo_preflight" | "environment_setup", isDependencySetupPreflight?: boolean): string;
