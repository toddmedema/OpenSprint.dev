...
function sectionKeyToHeader(key) {
...
function headerToSectionKey(header) {
...
export function prdToSpecMarkdown(prd) {
  const lines = ["# Product Specification", ""];
...
export function specMarkdownToPrd(markdown, metadata) {
  const sections = {};
  ...
  let lastIndex = 0;
  let lastKey = null;
  const normalizeContent = (raw) => (raw.trim() === "_No content yet_" ? "" : raw.trim());
  let match;
...
//# sourceMappingURL=spec-serializer.js.map
