...
function migrateDeploymentConfig(raw) {
    ...
    if (input.envVars && Object.keys(input.envVars).length > 0) {
        ...
        if (existingTargets.length > 0) {
            const defaultIdx = existingTargets.findIndex((t) => t.isDefault) >= 0
                ? existingTargets.findIndex((t) => t.isDefault)
                : 0;
            const migratedTargets = existingTargets.map((t, i) => i === defaultIdx ? { ...t, envVars: { ...t.envVars, ...input.envVars } } : t);
...
export function aiAutonomyLevelFromHilConfig(hilConfig) {
    ...
    const allRequireApproval = scopeChanges === "requires_approval" &&
        architectureDecisions === "requires_approval" &&
        dependencyModifications === "requires_approval";
    if (allRequireApproval)
        return "confirm_all";
    const majorRequireApproval = scopeChanges === "requires_approval" &&
        architectureDecisions === "requires_approval" &&
        dependencyModifications === "automated";
    if (majorRequireApproval)
        return "major_only";
    return "full";
}
...
export const REVIEW_AGENT_OPTIONS = [{ value: GENERAL_REVIEW_OPTION, label: "General" }, ...REVIEW_ANGLE_OPTIONS];
...
export const VALID_SELF_IMPROVEMENT_FREQUENCIES = [
    "never",
    "after_each_plan",
    "daily",
    "weekly",
...
export function validateDatabaseUrl(url) {
    ...
    throw new Error("databaseUrl must start with postgres:// or postgresql://, or be a SQLite path (sqlite://, file://, or path ending in .sqlite/.db)");
}
/**
...
export function parseSettings(raw) {
    ...
    const gitWorkingMode = r?.gitWorkingMode === "worktree" || r?.gitWorkingMode === "branches"
        ? r.gitWorkingMode
        : "worktree";
    const mergeStrategy = typeof r?.mergeStrategy === "string" &&
        VALID_MERGE_STRATEGIES.includes(r.mergeStrategy)
        ? r.mergeStrategy
        : "per_task";
    let aiAutonomyLevel = DEFAULT_AI_AUTONOMY_LEVEL;
    const rawLevel = r?.aiAutonomyLevel;
    if (typeof rawLevel === "string" &&
        VALID_AI_AUTONOMY_LEVELS.includes(rawLevel)) {
    ...
    const selfImprovementFrequency = typeof r?.selfImprovementFrequency === "string" &&
    ...
    const base = {
        ...
        selfImprovementLastRunAt: typeof r?.selfImprovementLastRunAt === "string" && r.selfImprovementLastRunAt.trim()
            ? r.selfImprovementLastRunAt.trim()
            : undefined,
        selfImprovementLastCommitSha: typeof r?.selfImprovementLastCommitSha === "string" && r.selfImprovementLastCommitSha.trim()
    ...
    const simple = (simpleObj && typeof simpleObj === "object" ? simpleObj : null) ??
        DEFAULT_AGENT;
    const complex = (complexObj && typeof complexObj === "object" ? complexObj : null) ??
        DEFAULT_AGENT;
    return {
        ...rest,
        simpleComplexityAgent: simple,
...
export function mergeApiKeysWithCurrent(incoming, current) {
    ...
    for (const provider of API_KEY_PROVIDERS) {
        ...
        for (const item of arr) {
            ...
            const limitHitAt = typeof e.limitHitAt === "string"
                ? e.limitHitAt
                : preserveExistingState
                    ? existing?.limitHitAt
                    : undefined;
            const invalidAt = typeof e.invalidAt === "string"
                ? e.invalidAt
                : preserveExistingState
                    ? existing?.invalidAt
                    : undefined;
            const label = typeof e.label === "string" ? e.label : preserveExistingState ? existing?.label : undefined;
            merged.push({
                id,
                value,
...
export function getProvidersInUse(settings) {
    ...
    for (const a of agents) {
        if (a.type === "claude" || a.type === "claude-cli")
            providers.add("ANTHROPIC_API_KEY");
        if (a.type === "cursor")
            providers.add("CURSOR_API_KEY");
        if (a.type === "openai")
            providers.add("OPENAI_API_KEY");
        if (a.type === "google")
            providers.add("GOOGLE_API_KEY");
    }
