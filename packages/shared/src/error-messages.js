...
export const ERROR_CODE_MESSAGES = {
    ...
    UNSUPPORTED_REPO_PATH: "Use a Linux path for the repo (e.g. /home/...). Windows-mounted WSL paths are not supported.",
    ...
    DEPENDENCY_SETUP_FAILED: "Run npm ci in the repository root, fix invalid dependencies, then retry.",
    ...
    DATABASE_UNAVAILABLE: "Database is unavailable. Check connection and retry; the app will retry automatically.",
 ...
 */
export function getMessageForErrorCode(code) {
    if (!code)
        return null;
...
export function getErrorCodeHint(code) {
 ...
 */
export function getQualityGateFailureLabel(failureType) {
 ...
 */
export function getQualityGateTitle(blocked) {
...
export const REPO_PREFLIGHT_REMEDIATION_GIT = "Fix repository git setup (base branch and git identity), then retry.";
/** Remediation for repo preflight when the failure is dependency-related. */
export const REPO_PREFLIGHT_REMEDIATION_DEPENDENCY = "Run npm ci in the repository root, then fix invalid dependencies before retrying.";
/** Remediation for environment_setup failure. */
export const ENVIRONMENT_SETUP_REMEDIATION = "Run npm ci in the repository root, re-link worktree node_modules, then retry.";
 ...
 */
export function getRemediationForFailureType(failureType, isDependencySetupPreflight) {
