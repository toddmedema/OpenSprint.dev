...
function formatAgentError(agentType: "claude" | "claude-cli" | "cursor" | "custom", raw: string): string {
...
export class AgentClient {
  ...
  private async invokeClaudeCli(options: AgentInvokeOptions): Promise<AgentResponse> {
    ...
    try {
      ...
      const raw = shape.stderr || shape.message || (error instanceof Error ? error.message : String(error));
  ...
  private async invokeCursorCli(options: AgentInvokeOptions): Promise<AgentResponse> {
    ...
    try {
      ...
      const raw = isTimeout
        ...
        : isAppErr
          ? error.message
          : execShape.stderr || execShape.message || (error instanceof Error ? error.message : String(error));
