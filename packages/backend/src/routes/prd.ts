...
export function createPrdRouter({ prdService, chatService }: PrdRouterDeps): Router {
  const prdRouter = Router({ mergeParams: true });
      ...
      switch (ext) {
        ...
        default:
          res.status(400).json({
            error: {
              code: "BAD_REQUEST",
              message: "Unsupported file type. Use .md, .docx, or .pdf",
            },
          });
          return;
      }
