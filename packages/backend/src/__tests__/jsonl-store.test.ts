...
describe("jsonl-store", () => {
  ...
  beforeEach(() => {
    ...
    repoPath = path.join(
      os.tmpdir(),
      `jsonl-store-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
  ...
  describe("updateIssue", () => {
    ...
    it("should throw AppError with ISSUE_NOT_FOUND when issue does not exist", async () => {
      await expect(updateIssue(repoPath, "nonexistent-id", { status: "closed" })).rejects.toThrow(
        AppError
      );
      await expect(
        updateIssue(repoPath, "nonexistent-id", { status: "closed" })
  ...
  describe("addDependency", () => {
    ...
    it("should throw AppError with ISSUE_NOT_FOUND when child does not exist", async () => {
      const parent = await createIssue(repoPath, "Parent", {});
      await expect(addDependency(repoPath, "nonexistent-child", parent.id)).rejects.toThrow(
        AppError
      );
