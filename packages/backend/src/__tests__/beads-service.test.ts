...
describe("BeadsService", () => {
  ...
  beforeEach(() => {
    ...
    clearStoreCache();
    clearJsonlCache();
    repoPath = path.join(os.tmpdir(), `beads-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    writeJsonl(repoPath, []);
  ...
  describe("areAllBlockersClosed", () => {
    ...
    it("should return true when all blockers are closed", async () => {
      writeJsonl(repoPath, [
        { id: "t1", status: "open", issue_type: "task", dependencies: [{ type: "blocks", depends_on_id: "b1" }] },
