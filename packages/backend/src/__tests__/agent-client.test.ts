...
vi.mock('util', () => ({
  promisify: (fn: (cmd: string, opts: unknown, cb: (err: Error | null, stdout?: string, stderr?: string) => void) => void) => {
...
describe('AgentClient', () => {
  ...
  describe('invoke', () => {
    it('should route cursor config to Cursor CLI', async () => {
      const mockChild = {
        killed: false,
        kill: vi.fn(),
        stdout: { on: vi.fn((_ev: string, fn: (d: Buffer) => void) => fn(Buffer.from('Cursor response'))) },
        stderr: { on: vi.fn() },
      ...
      expect(mockSpawn).toHaveBeenCalledWith('agent', expect.any(Array), expect.objectContaining({ cwd: '/tmp' }));
  ...
  describe('spawnWithTaskFile', () => {
    it('should spawn Cursor agent with task content and workspace', async () => {
      ...
      await fs.mkdir(path.dirname(path.join(tmpDir, '.opensprint/active/bd-a3f8.1/prompt.md')), { recursive: true });
      ...
      expect(mockSpawn).toHaveBeenCalledWith('agent', expect.arrayContaining(['--print', '--workspace', cwd, '--trust']), expect.any(Object));
    ...
    it('should spawn custom agent with cliCommand and task file path', () => {
      ...
      expect(mockSpawn).toHaveBeenCalledWith('my-cli', ['--verbose', taskFilePath], expect.objectContaining({ cwd }));
    });
