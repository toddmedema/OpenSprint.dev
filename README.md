...
**Desktop local API token:** In `OPENSPRINT_DESKTOP=1` mode the backend embeds the rotating local Bearer token in the served `index.html` under a strict Content-Security-Policy nonce, with `Cache-Control: no-store` on that document. Electron’s main process and other non-browser clients may still read a small fallback JavaScript URL; browsers attach `Sec-Fetch-Site`, and that URL refuses `cross-site` and `same-site` fetches so another localhost origin cannot trivially scrape the token. This is not a substitute for locking down a shared machine: anyone who can run arbitrary local code or a malicious browser extension may still capture the token—desktop mode assumes mutual trust with localhost on a typical single-user workstation.
